...
export function CapaContent({ isPage=false }) {
  ...
  return <section className={isPage?"capa-page":"capa-modal"} aria-labelledby="capa-title">
    ...
    <div className="capa-description"><p className="capa-description-kicker">{t("capa.firstCongress")}</p><h3>{t("capa.descriptionTitle")}</h3><p>{t("capa.description")}</p><div className="capa-ticket-row"><div className="capa-ticket-actions"><a href="https://crossingoverlatam.bio/" target="_blank" rel="noopener noreferrer">{t("capa.tickets")} <span aria-hidden="true">↗</span></a><a className="capa-streaming-ticket" href="https://www.passline.com/view-event/congreso-argentino-de-proteinas-alternativas?utm_source=chatgpt.com" target="_blank" rel="noopener noreferrer" aria-label={t("capa.streamingTicketLabel")}>{t("capa.streamingTicket")} <span aria-hidden="true">↗</span></a></div><div className="capa-discount" aria-label={t("capa.discountLabel")}><span>{t("capa.discount")}</span><strong>CAPAXO</strong></div></div></div>
    <CapaSchedule />
  </section>;
}
