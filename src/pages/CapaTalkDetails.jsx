...
const languageNames = {
  es: { ES: "Español", EN: "Inglés", PT: "Portugués", IT: "Italiano" },
  en: { ES: "Spanish", EN: "English", PT: "Portuguese", IT: "Italian" },
};
  ...
  return (
    <article className="capa-talk-detail">
      ...
      <div className="talk-detail-layout">
        <div className="talk-speaker-panels">{speakers.map((speaker) => { const organizationLogo = getOrganizationLogo(speaker.company || talk.company, speaker.logo); const country = speaker.country || talk.country; const activePhoto = talk.day === "wednesday" && speaker.cleanPhoto ? speaker.cleanPhoto : speaker.photo; return <aside className="talk-speaker-panel" key={speaker.name}>{talk.logoOnly && organizationLogo ? <img className="contain" src={organizationLogo} alt={`Logo de ${talk.company}`} /> : activePhoto ? <img className={speaker.photoFit === "contain" ? "contain" : ""} src={activePhoto} alt={speaker.name} /> : <div className="talk-photo-placeholder large"><span>{speaker.name.split(" ").map((part) => part[0]).slice(0,2).join("")}</span></div>}<p>{c.speaker}</p><h2>{speaker.name}</h2><h3>{speaker.company || talk.company}{country && <span className={`talk-flag ${Array.isArray(country) ? "multiple" : ""}`}><CountryFlag country={country} /></span>}</h3><div className="talk-speaker-logo">{organizationLogo ? <img src={organizationLogo} alt={`Logo de ${speaker.company || talk.company}`} /> : <span>{speaker.company || talk.company}</span>}</div><dl><div><dt>{c.schedule}</dt><dd>{talk.date} · {talk.time}{talk.timezone ? ` · ${talk.timezone}` : ""}</dd></div><div><dt>{c.room}</dt><dd>{talk.room}</dd></div><div><dt>{c.mode}</dt><dd>{talk.mode === "TBD" ? "TBD" : talk.mode === "remoto" ? c.remote : c.inPerson}</dd></div><div><dt>{c.language}</dt><dd>{(Array.isArray(talk.language) ? talk.language : [talk.language]).map((language) => languageNames[locale][language] || language).join(" · ")}</dd></div></dl></aside>; })}{talk.detailLogo && <div className="talk-detail-institution-logo"><img src={talk.detailLogo} alt={`Logo de ${talk.company}`} /></div>}{talk.detailLogos?.map((logo) => <div className="talk-detail-institution-logo" key={logo.name}><img src={logo.src} alt={`Logo de ${logo.name}`} /></div>)}</div>
