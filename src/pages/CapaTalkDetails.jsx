  ...
  return (
    <article className="capa-talk-detail">
      ...
      <div className="talk-detail-layout">
        <div className="talk-speaker-panels">{speakers.map((speaker) => { const organizationLogo = getOrganizationLogo(speaker.company || talk.company, speaker.logo); return <aside className="talk-speaker-panel" key={speaker.name}>{speaker.photo ? <img src={speaker.photo} alt={speaker.name} /> : <div className="talk-photo-placeholder large"><span>{speaker.name.split(" ").map((part) => part[0]).slice(0,2).join("")}</span></div>}<p>{c.speaker}</p><h2>{speaker.name}</h2><h3>{speaker.company || talk.company}</h3><div className="talk-speaker-logo">{organizationLogo ? <img src={organizationLogo} alt={`Logo de ${speaker.company || talk.company}`} /> : <span>{speaker.company || talk.company}</span>}</div><dl><div><dt>{c.schedule}</dt><dd>{talk.date} · {talk.time}</dd></div><div><dt>{c.room}</dt><dd>{talk.room}</dd></div><div><dt>{c.mode}</dt><dd>{talk.mode === "remoto" ? c.remote : c.inPerson}</dd></div><div><dt>{c.language}</dt><dd>{languageNames[talk.language] || talk.language}</dd></div></dl></aside>; })}{talk.detailLogo && <div className="talk-detail-institution-logo"><img src={talk.detailLogo} alt={`Logo de ${talk.company}`} /></div>}</div>
