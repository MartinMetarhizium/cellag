...
export default function CapaTalkDetails() {
  ...
  return (
    <article className="capa-talk-detail">
      <Link className="talk-detail-back" to={returnTo}>← {c.back}</Link>
      <header className={`talk-detail-hero category-accent-${talk.category.toLowerCase()}`}><div className="talk-detail-meta"><span>{(typeof talk.categoryLabel === "object" ? talk.categoryLabel[locale] : talk.categoryLabel) || categoryNames[locale][talk.category] || talk.category}</span><span>{languageNames[talk.language] || talk.language}</span><span>{talk.mode === "remoto" ? "💻" : "🎤"} {talk.mode === "remoto" ? c.remote : c.inPerson}</span></div><h1>{talk.title}</h1></header>
      <div className="talk-detail-layout">
        <div className="talk-speaker-panels">{speakers.map((speaker) => <aside className="talk-speaker-panel" key={speaker.name}>{speaker.photo ? <img src={speaker.photo} alt={speaker.name} /> : <div className="talk-photo-placeholder large"><span>{speaker.name.split(" ").map((part) => part[0]).slice(0,2).join("")}</span></div>}<p>{c.speaker}</p><h2>{speaker.name}</h2><h3>{speaker.company || talk.company}</h3><dl><div><dt>{c.schedule}</dt><dd>{talk.date} · {talk.time}</dd></div><div><dt>{c.room}</dt><dd>{talk.room}</dd></div><div><dt>{c.mode}</dt><dd>{talk.mode === "remoto" ? c.remote : c.inPerson}</dd></div><div><dt>{c.language}</dt><dd>{languageNames[talk.language] || talk.language}</dd></div></dl></aside>)}{talk.detailLogo && <div className="talk-detail-institution-logo"><img src={talk.detailLogo} alt={`Logo de ${talk.company}`} /></div>}</div>
        <main className="talk-detail-content"><section><p className="talk-detail-kicker">{c.about}</p>{talk.description.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section><section><p className="talk-detail-kicker">{c.topics}</p><ul>{talk.topics.map((topic) => <li key={topic}>{topic}</li>)}</ul></section>{talk.keywords?.length > 0 && <section><p className="talk-detail-kicker">{c.keywords}</p><div className="talk-detail-keywords">{talk.keywords.map((keyword) => <span key={keyword}>{keyword}</span>)}</div></section>}</main>
      </div>
    </article>
  );
}
